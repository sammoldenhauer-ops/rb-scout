...
const content = readFileSync('C:/Users/User/Desktop/rb-scout/data/playersBase.js', 'utf8');

// Extract ALL_PLAYERS_BASE JSON block
const startIdx = content.indexOf('export const ALL_PLAYERS_BASE = {');
...
console.log('\nDynamic ideal values (weighted avg from top24>0 players):');
...
metrics.forEach(m => {
  if (wts[m] > 0) {
    ...
    const flag = diff > 0.5 ? ' <<< DIFFERS FROM DEFAULT' : '';
    console.log(`  ${m.padEnd(12)}: dynamic=${v.toFixed(3)}  default=${DEFAULT_IDEALS[m]}${flag}`);
  } else {
    console.log(`  ${m.padEnd(12)}: no data   default=${DEFAULT_IDEALS[m]}`);
  }
});
